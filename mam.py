...
class CurveCALayerrh(nn.Module):
    ...
    def forward(self, x):
        ...
        return x
# class CurveCALayerrh(nn.Module):
#     def __init__(self, channel, n_curve=8):  # 添加 n_curve 参数
#         super(CurveCALayerrh, self).__init__()
#         self.avg_pool = nn.AdaptiveAvgPool2d(1)
#         self.n_curve = n_curve  # 使用传入的 n_curve 参数
#         self.relu = nn.ReLU(inplace=False)
#
#         # 使用多尺度特征融合模块
#         self.multi_scale_feature_fusion = LightweightMultiScaleFeatureFusion(channel)
...
# 光照感知模块
class IlluminationPerceptionModule(nn.Module):
    def __init__(self):
        super(IlluminationPerceptionModule, self).__init__()
        self.illumination_conv = nn.Conv2d(3, 1, kernel_size=1, stride=1, padding=0)
        self.sigmoid = nn.Sigmoid()

    def forward(self, x):
        illumination = self.illumination_conv(x)
        illumination = self.sigmoid(illumination)
        return illumination
